...
MEMORY USED:
- [bank path § section] -> [the step, constraint, or verify check it changed] (or: none)

MEMORY CONFLICTS:
- [bank path § section] [current fact | durable decision | intended future | open question]: [conflict] -> [what the plan does about it] (or: none)
```
